...
func (u *EnrollmentRepo) CreateEnrollment(enrollment *model.Enrollment) error {
	...
	_, err = tx.Exec(query, enrollment.UserId, enrollment.CourseId, time.Now())
...
func (u *EnrollmentRepo) UpdateEnrollment(enrollment *model.Enrollment) error {
	...
	result, err := tx.Exec(query, enrollment.UserId, enrollment.CourseId, enrollment.EnrollmentDate,
...
func (u *EnrollmentRepo) DeleteEnrollment(id string) error {
	...
	if affectedRows == 0 {
		return fmt.Errorf("no rows have been affected no such enrollment")
	}
