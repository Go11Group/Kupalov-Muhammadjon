...
func (u *LessonRepo) GetLessons(filter model.LessonFilter) (*[]model.Lesson, error) {
	query := `
	select 
		lesson_id, name, email, birthday, password, created_at, updated_at
	from
		lessons
	where 
		deleted_at is null
...
func (u *LessonRepo) UpdateLesson(lesson *model.Lesson) error {
	...
	set
		...
		content = $3
	where
		deleted_at is null and lesson_id = $4
	`
	tx, err := u.Db.Begin()
	defer tx.Commit()
	...
	_, err = tx.Exec(query, lesson.CourseId, lesson.Title, lesson.Content, time.Now(), lesson.LessonId)
