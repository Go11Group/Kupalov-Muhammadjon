...
func (h *Handler) GetPopularCourses(c *gin.Context) {
	...
	sd, hasKey := c.GetQuery("start_date")
	if hasKey {
		startDate, err := time.Parse("2006-01-02", sd)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{
				"Error":   "Bad request",
				"Message": "Error in parsing start_date to time",
			})
			log.Println("Error in parsing start_date to time", err)
			return
		}
		...
		c.JSON(http.StatusBadRequest, gin.H{
			"Error":   "Bad request",
			"Message": "Error startDate is missing",
		})
		log.Println("Error startDate is missing")
		return
	}
	ed, hasKey := c.GetQuery("end_date")
	if hasKey {
		endDate, err := time.Parse("2006-01-02", ed)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{
				"Error":   "Bad request",
				"Message": "Error in parsing end_date to time",
			})
			log.Println("Error in parsing end_date to time", err)
			return
		}
		...
		c.JSON(http.StatusBadRequest, gin.H{
			"Error":   "Bad request",
			"Message": "Error endDate is missing",
		})
		log.Println("Error endDate is missing")
		return
	}
	lms, hasKey := c.GetQuery("limit")
	if hasKey {
	...
	courses, err := h.CourseRepo.GetPopularCourses(&filter)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"Error":   "Bad request",
			"Message": "Error while getting popular courses",
		})
		log.Println("Error while getting popular courses ", err)
		return
	}
	c.JSON(200, courses)
}
